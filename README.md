...
- **Гарантированные слои модальных окон** — браузер и все диалоги менеджера всегда выше
  чужих оверлеев (подробнее в разделе «Слои модальных окон»)

### Слои модальных окон (z-index)

Менеджер использует выделенные слои поверх шкалы ядра MoonShine, поэтому его окна
**всегда** выше сторонних оверлеев на модальном слое ядра (1100) — например,
picker'а `moonshine-flexible-layouts`:
...
Значения наследуются от токенов ядра через `calc()`, поэтому при смене шкалы ядра
слои пересчитаются автоматически. Оба токена можно переопределить в CSS хоста.
