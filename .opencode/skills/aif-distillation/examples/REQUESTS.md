...
- `.opencode/skills/ddd-practices/SKILL.md`
- `.opencode/skills/ddd-practices/references/SOURCE-MAP.md`
...
- write each child directly under `.opencode/skills/<prefix>-<child-scope>/`
...
- no writes to the current agent `.opencode/skills`
...
- save to `.opencode/skills/platform-operator/`
...
- confirm the resolved target path would stay under the resolved output root (`.opencode/skills` by default, or `--path` when present)
